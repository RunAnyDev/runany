...
**A:** Yes. Set `stream: true` in the `response.create` call to receive server-sent events. The the project docs show a `stream: false` example, but streaming is supported.

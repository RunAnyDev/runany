...
**Q: How fast is it?**
**A:** The the project docs show ~4 tok/s on 6x RTX 5090 with a full expert hot tier. On CPU-only with NVMe streaming, expect lower throughput. The authors note that insufficient fast memory may reduce speed but the default policy never silently changes model precision.
